...
class StrengthAnalyser:
    ...
    def _calculate_entropy(self, password_length: int, pool_size: int) -> float:
        """Entropy formula used in the IA specification."""
    ...
    def _contains_sequential_pattern(self, password: str) -> bool:
        """Detect ascending/descending alphabetic or numeric runs.

        We flag runs of length >= 3 because short 2-char sequences are common
        and create too many false positives in realistic passwords.
        """
    ...
    def _contains_repeated_pattern(self, password: str) -> bool:
        """Return True when same character repeats at least 3 times consecutively."""
        return re.search(r"(.)\1\1+", password) is not None

    def _contains_repeated_block_pattern(self, password: str) -> bool:
        """Return True if the full password is built from a repeating substring.

        Examples: `abababab`, `abcabcabc`, `12121212`.
        """
        if len(password) < 6:
            return False
    ...
    def _contains_keyboard_walk(self, password: str) -> bool:
        """Identify 3+ character keyboard walks across known key rows.

        Keyboard patterns are popular because they are easy to remember,
        but attackers explicitly include these paths in cracking rules.
        """
        lowered = password.lower()
        if len(lowered) < 3:
            return False
    ...
    def _is_common_password(self, password: str) -> bool:
        """Check exact-match against local common-password reference list."""
        return password.lower() in self.common_passwords

    def _contains_dictionary_word(self, password: str) -> bool:
        """Check if password embeds obvious dictionary-like words.

        This is intentionally basic and deterministic for explainability.
        """
        lowered = password.lower()
    ...
    def _apply_entropy_penalties(
        ...
        """Reduce effective entropy when predictable cracking patterns are present.

        Pure character-set entropy can overestimate real security for passwords
        that are common, dictionary-based, or pattern-based. Penalties adjust
        the score to better reflect practical attacker behavior.
        """
        penalty = 0.0
