...
class DashboardFrame(ctk.CTkFrame):
    ...
    def _build_layout(self) -> None:
        ...
        self.list_container.grid(row=2, column=0, sticky="nsew", padx=16, pady=(0, 16))
    ...
    def populate_entries(self, entries: list[dict]) -> None:
        ...
        for row_index, entry in enumerate(entries, start=1):
            ctk.CTkRadioButton(
                ...
                value=str(entry["id"]),
            ).grid(row=row_index, column=0, padx=8, pady=8, sticky="w")
    ...
    def set_status_message(self, message: str, is_error: bool = False) -> None:
        """Show a brief status message in the header search box placeholder."""
        placeholder = message if message else "Search by service, username, or category..."
        # Placeholder text is used to keep UI compact without adding extra labels.
        for widget in self.winfo_children():
            if isinstance(widget, ctk.CTkFrame):
                for child in widget.winfo_children():
                    if isinstance(child, ctk.CTkEntry):
                        child.configure(placeholder_text=placeholder)
                        if is_error:
                            child.configure(border_color="#ff6b6b")
                        else:
                            child.configure(border_color="#565b5e")
                        return
