...
class BreachChecker:
    ...
    def check_password(self, password: str) -> dict[str, Any]:
        """Return breach count for a password using SHA-1 prefix search.

        The API receives only the first 5 hex chars of the hash. Matching is
        performed locally, which preserves user privacy by design.
        """
        if not password:
            return {
                "checked": False,
